...
void CZMQNotificationInterface::BlockConnected(std::shared_ptr<CBlock>& pblock, const CBlockIndex* pindexConnected)
{
...
void CZMQNotificationInterface::BlockDisconnected(std::shared_ptr<CBlock>& pblock, const CBlockIndex* pindexDisconnected)
{
    for (const CTransactionRef& ptx : pblock->vtx) {
        const CTransaction& tx = *ptx;
        TryForEachAndRemoveFailed(notifiers, [&tx](CZMQAbstractNotifier* notifier) {
