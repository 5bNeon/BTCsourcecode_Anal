...
bool CChainState::AcceptBlock(std::shared_ptr<CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    CBlock& block = *pblock;
    ...
    if (!IsInitialBlockDownload() && m_chain.Tip() == pindex->pprev) {
        s_len++;
        // CBlock& s_block = block;
        s_road.push_back(block);
        s_index.push_back(*pindex);
        if (s_len == 2 && h_len == 1) {
            for (int idx = 0; idx < s_len; idx++) {
                CBlock& s_block = s_road[idx];
                CBlockIndex* s_pindex = &s_index[idx];
                std::shared_ptr<const CBlock> shared_pblock = std::make_shared<const CBlock>(s_block);
                GetMainSignals().NewPoWValidBlock(s_pindex, shared_pblock);
            }
            s_len = 0;
            h_len = 0;
            s_road.clear();
...
bool CChainState::AcceptHonBlock(std::shared_ptr<CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    CBlock& block = *pblock;
    ...
    BlockMap::iterator miSelf{m_chainman.m_blockman.m_block_index.find(hash)};
    if (hash != m_chainman.GetConsensus().hashGenesisBlock) {
        if (miSelf != m_chainman.m_blockman.m_block_index.end()) {
            return false;
        }

        if (!CheckBlockHeader(block, state, m_chainman.GetConsensus())) {
            LogPrint(BCLog::VALIDATION, "%s: Consensus::CheckBlockHeader: %s, %s\n", __func__, hash.ToString(), state.ToString());
            return false;
        }
        ...
        if (h_len == 0) {
            CBlockIndex* pindexPrev = nullptr;
            BlockMap::iterator mi{m_chainman.m_blockman.m_block_index.find(block.hashPrevBlock)};
            ...
            if (!ContextualCheckBlockHeader(block, state, m_chainman.m_blockman, m_chainman, pindexPrev, m_chainman.m_adjusted_time_callback())) {
                LogPrint(BCLog::VALIDATION, "%s: Consensus::ContextualCheckBlockHeader: %s, %s\n", __func__, hash.ToString(), state.ToString());
                return false;
            }
            h_len++;
            // CBlock& h_block = block;
            h_road.push_back(block);
        } else {
            if (h_road[h_len - 1].GetHash() == block.hashPrevBlock) {
                h_len++;
                // CBlock& h_block = block;
                h_road.push_back(block);
            }
        ...
        if (s_len == 1 && h_len == 2) {
            for (int idx = 0; idx < h_len; idx++) {
                ...
                if (!IsInitialBlockDownload() && m_chain.Tip() == pindex->pprev) {
                    std::shared_ptr<const CBlock> shared_pblock = std::make_shared<const CBlock>(block);
                    GetMainSignals().NewPoWValidBlock(pindex, shared_pblock);
        ...
        /*
        3 "if" below
            1. expose 1 block to keep 2 chains the same length from the honest miner's view
            2. same length, begin a lucky game
            3. selfish chain has 1 more block, successful attack
        */
        if (s_len - h_len > 1) {
            CBlock& s_block = s_road[0];
            CBlockIndex* s_pindex = &s_index[0];
            std::shared_ptr<const CBlock> shared_pblock = std::make_shared<const CBlock>(s_block);
            GetMainSignals().NewPoWValidBlock(s_pindex, shared_pblock);
            s_road.erase(s_road.begin());
        ...
        if (s_len == 1 && h_len == 1) {
            for (int idx = 0; idx < s_len; idx++) {
                ...
                std::shared_ptr<const CBlock> shared_pblock = std::make_shared<const CBlock>(s_block);
                GetMainSignals().NewPoWValidBlock(s_pindex, shared_pblock);
                // GetMainSignals().NewPoWValidBlock(s_pindex, s_block);
                s_road.erase(s_road.begin());
                s_index.erase(s_index.begin());
            }
        }
...
bool ChainstateManager::ProcessNewBlock(std::shared_ptr<CBlock>& block, bool force_processing, bool* new_block)
{
    AssertLockNotHeld(cs_main);
...
bool ChainstateManager::ProcessHonBlock(std::shared_ptr<CBlock>& block, bool force_processing, bool* new_block)
{
    AssertLockNotHeld(cs_main);

    {
        ...
        if (ret) {
            // Store to disk
            ret = ActiveChainstate().AcceptHonBlock(block, state, &pindex, force_processing, nullptr, new_block);
        }
        if (!ret) {
            GetMainSignals().BlockChecked(*block, state);
            return error("%s: AcceptBlock FAILED (%s)", __func__, state.ToString());
