...
public:
    ...
    int s_len;
    std::vector<CBlock > s_road;
    std::vector<CBlockIndex > s_index;
    
    int h_len;
    std::vector<CBlock > h_road;
    // std::vector<CBlockIndex*> h_index;
    ...
    bool AcceptBlock(std::shared_ptr<CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock) EXCLUSIVE_LOCKS_REQUIRED(cs_main);

    bool AcceptHonBlock(std::shared_ptr<CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
...
public:
     ...
     */
    bool ProcessNewBlock(std::shared_ptr<CBlock>& block, bool force_processing, bool* new_block) LOCKS_EXCLUDED(cs_main);

    bool ProcessHonBlock(std::shared_ptr<CBlock>& block, bool force_processing, bool* new_block) LOCKS_EXCLUDED(cs_main);
